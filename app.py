...
def actualizar_comparacion(estados, n_clicks, *args):
    ...
    for cat in categories:
        if cat in df_radar.columns:
            ...
            temp_df = df[[cat]].copy()
            df_radar[f"{cat}_norm"] = scaler.fit_transform(temp_df)
    
    # Aplicar inversión y ponderación
    for i, cat in enumerate(categories):
        if cat in invertir:
            df_radar[f"{cat}_norm"] = 1 - df_radar[f"{cat}_norm"]
        
        # Aplicar peso
        df_radar[f"{cat}_norm"] = df_radar[f"{cat}_norm"] * pesos[i]
    ...
    for i, estado in enumerate(estados):
        ...
        for cat in categories:
            if cat in df_radar.columns and f"{cat}_norm" in df_radar.columns:
                # Asegurarse de que el estado existe en el dataframe
                if estado in df_radar["Estado"].values:
                    val = df_radar[df_radar["Estado"] == estado][f"{cat}_norm"].values[0]
                    values.append(val)
...
def actualizar_mapa(indicador, n_clicks, *args):
    ...
    for estado in df_mapa["Estado"].unique():
        if estado in mexico_states_coords:
            ...
            valor_min = df_mapa[valor_mostrar].min()
            valor_max = df_mapa[valor_mostrar].max()
            valor_norm = (valor - valor_min) / (valor_max - valor_min) if valor_max > valor_min else 0.5
            
            # Obtener color del mapa de colores
            cmap = cm.get_cmap('viridis')
            color = mcolors.rgb2hex(cmap(valor_norm))
    ...
    fig.update_geos(
        ...
        showsubunits=True,
        subunitcolor='Black',
        lataxis={'range': [14, 33]},  # Ajustar para México
        lonaxis={'range': [-118, -86]},  # Ajustar para México
    )
...
# Callback para actualizar los gráficos en el modo draggable
@app.callback(
    [Output("graph-ranking-drag", "figure"),
     Output("graph-mapa-drag", "figure"),
     Output("graph-comparar-drag", "figure"),
     ...
     Output("top-estados-drag", "children")],
    [Input("btn-calcular-drag", "n_clicks")],
    [State(f"slider-drag-{crit}", "value") for crit in criterios.keys()] + 
     [State("checklist-invertir-drag", "value"),
      State("dropdown-mapa-drag", "value"),
      State("dropdown-comparar-drag", "value"),
      State("dropdown-indicador-drag", "value")]
)
def actualizar_graficos_draggable(n_clicks, *args):
    if n_clicks is None:
        # Valores predeterminados para la carga inicial
        return [go.Figure()] * 5 + [html.Div()]
    
    # Extraer los argumentos
    pesos = args[:-4]  # Todos los argumentos excepto los últimos 4
    invertir = args[-4]  # Checklist invertir
    mapa_indicador = args[-3]  # Dropdown mapa
    comparar_estados = args[-2]  # Dropdown comparar
    indicador = args[-1]  # Dropdown indicador
    ...
    # 1. Crear gráfico de barras para el ranking
    fig_ranking = px.bar(
        df_ranking,
        x="Estado",
        y="puntuacion_final",
    ...
    # 2. Crear mapa de México
    # Determinar si mostrar el indicador seleccionado o la puntuación final
    if mapa_indicador == "puntuacion_final":
        # Usar df_calc para el mapa
        df_mapa = df_calc
        valor_mostrar = "puntuacion_final"
        titulo = "Puntuación Final por Estado"
        ...
        # Mostrar el indicador seleccionado
        df_mapa = df
        valor_mostrar = mapa_indicador
        titulo = f"{criterios[mapa_indicador]} por Estado"
    ...
    # 3. Crear gráfico de comparación de estados
    if not comparar_estados:
        comparar_estados = df["Estado"].unique()[:5].tolist()
    
    # Filtrar el dataframe para los estados seleccionados
    df_comp = df[df["Estado"].isin(comparar_estados)]
    ...
    # Crear gráfico de radar
    fig_comparar = go.Figure()
    
    # Colores para los diferentes estados
    colores = px.colors.qualitative.Plotly[:len(comparar_estados)]
    
    for i, estado in enumerate(comparar_estados):
        values = []
        theta_labels = []
        ...
        if values:
            ...
            fig_comparar.add_trace(go.Scatterpolar(
                r=values,
                theta=theta_labels,
                fill='toself',
                name=estado,
    ...
    # 4. Crear gráfico de indicador por estado
    # Ordenar por el indicador seleccionado
    df_sorted = df.sort_values(indicador, ascending=False)
    ...
    # 6. Crear componente para mostrar los top 5 estados
    top5 = df_ranking.head(5)
    ...
    return fig_ranking, fig_mapa, fig_comparar, fig_indicador, fig_correlacion, top_estados_cards
